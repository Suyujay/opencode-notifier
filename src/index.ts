...
export interface NotifierSessionClient {
  ...
  /** True when the permission request is still awaiting a reply. Fails open (true) when unknown. */
    ...
    switch (event.type) {
      ...
      case "permission.asked": {
        ...
        if (permissionID) {
          ...
          stillPending = await client.permissionPending(sessionID, permissionID)
        }
        ...
        if (stillPending && !shouldSuppressPermissionAlert(sessionID)) {
          await handleEventWithElapsedTime(client, config, "permission", projectName, sessionID)
        }
        return
      }
...
function createV2SessionClient(ctx: OpenCodeContext): NotifierSessionClient {
  return {
    ...
    async permissionPending(sessionID: string | null, permissionID: string): Promise<boolean> {
      ...
      try {
        const pending = await ctx.permission.list({ sessionID } as Parameters<typeof ctx.permission.list>[0])
