...
describe("normalizeV1Event", () => {
  ...
  test("carries the permission ID from permission.asked", () => {
    const events = normalizeV1Event({ type: "permission.asked", properties: { sessionID: "s-1", id: "per-1" } })

    expect(events).toEqual([{ type: "permission.asked", sessionID: "s-1", permissionID: "per-1" }])
  })
...
describe("normalizeV2Event", () => {
  ...
  test("carries the permission ID from the data envelope", () => {
    expect(normalizeV2Event({ type: "permission.asked", data: { sessionID: "s-1", id: "per-1" } })).toEqual([
      { type: "permission.asked", sessionID: "s-1", permissionID: "per-1" },
    ])
  })
