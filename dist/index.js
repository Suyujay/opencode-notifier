...
function v1SessionClient(client) {
  return {
    ...
    async isPermissionPending(permissionID) {
      return isPermissionStillPending(client, permissionID);
    }
  };
}
function v2SessionClient(ctx) {
  return {
    async listMessages(sessionID) {
      try {
        ...
        return list.map((msg) => {
          ...
          const time = info?.time ?? {};
          const created = typeof time.created === "number" ? time.created : typeof time.start === "number" ? time.start : undefined;
          return {
            role: typeof info.role === "string" ? info.role : undefined,
            created
          };
    ...
    async isPermissionPending(permissionID, sessionID) {
      try {
        const pending = sessionID ? await ctx.permission.list({ sessionID }) : await ctx.permission.list();
        const list = Array.isArray(pending) ? pending : Array.isArray(pending?.data) ? pending.data : Array.isArray(pending?.requests) ? pending.requests : null;
        if (!list)
          return true;
        return list.some((p) => p?.id === permissionID || p?.requestID === permissionID);
      } catch {
        return true;
      }
    }
...
async function getElapsedSinceLastPrompt(api, sessionID, nowMs = Date.now()) {
  try {
...
async function processSessionIdle(api, config, projectName, event, sessionID, sequence, idleReceivedAtMs) {
...
function scheduleSessionIdle(api, config, projectName, event, sessionID) {
  clearPendingIdleTimer(sessionID);
  const sequence = bumpSessionIdleSequence(sessionID);
  const idleReceivedAtMs = Date.now();
  const timer = setTimeout(() => {
    pendingIdleTimers.delete(sessionID);
    processSessionIdle(api, config, projectName, event, sessionID, sequence, idleReceivedAtMs).catch(() => {
      return;
    });
  }, IDLE_COMPLETE_DELAY_MS);
  pendingIdleTimers.set(sessionID, timer);
}
async function handleEventWithElapsedTime(api, config, eventType, projectName, event, elapsedReferenceNowMs, preloadedSessionTitle) {
...
async function handleServerEvent(api, projectName, isCLI, event) {
...
async function setupV2(ctx) {
  ...
  const isCLI = isCLIClient(clientEnv);
  if (isCLI) {
    handleEvent(loadConfig(), "client_connected", projectName, null);
  } else {
    setTimeout(() => {
      handleEvent(loadConfig(), "client_connected", projectName, null);
    }, 100);
  ...
  try {
    await ctx.tool.hook("execute.before", (hookEvent) => {
      const config = loadConfig();
      const tool = hookEvent?.tool;
      if (tool === "question") {
        handleEvent(config, "question", projectName, null);
      }
      if (tool === "plan_exit") {
        handleEvent(config, "plan_exit", projectName, null);
      }
    });
  } catch {}
